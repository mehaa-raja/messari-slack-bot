...
QUESTION: Is "${company}" the main subject performing a meaningful business action (like raising funds, launching products, forming partnerships, making acquisitions, etc.)?

RESPOND WITH ONLY:
- "MEANINGFUL" if ${company} is the main actor doing something significant
- "PASSIVE" if ${company} is just mentioned as infrastructure, platform, or background context
- "PRICE_ONLY" if it's only about ${company}'s token price changes

Examples:
- "Circle announces new partnership" → MEANINGFUL
- "Token launches on Ethereum blockchain" → PASSIVE (Ethereum just infrastructure)
- "Solana validator launched by third party" → PASSIVE (Solana just platform)
...
export async function detectPortfolioMentionsInText(rawContent, portfolioList = PANTERA_PORTFOLIO_COMPANIES) {
  ...
  for (const article of articles) {
    ...
    for (const subject of mainSubjects) {
      ...
      // Check if this is a meaningful business action (not price-only)
      if (isMeaningfulBusinessAction(article, actionSummary)) {
        // AI-powered validation step
        ...
        console.log(`   🚫 ${company} filtered out - not meaningful business action`);
      }
    }
  }
