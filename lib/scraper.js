    ...
    const response = await axios.post('https://api.messari.io/ai/openai/chat/completions', {
      messages: [
        ...
        },
        {
          ...
          8. PANTERA PORTFOLIO COMPANY NEWS: Specifically look for and include any meaningful business developments from these Pantera Capital portfolio companies: 0x, 1inch, Abra, Acala Network, Alchemy, Ampleforth, Anchor, Anchorage, Ancient8, Ankr, API3, Arbitrum, Audius, Aurora, Bakkt, Balancer Labs, BitDAO, BitGo, Bitstamp, Bittensor, Bitwise, Blockchain.com, Circle, Civic, CoinDCX, Cosmos Network, Enigma, ErisX, Ethena, Expand, Filecoin, Flashbots, Flexa, Gemini, Handshake, Injective Protocol, Lido, Liquity, Livepeer, Mina Protocol, Mirror Protocol, Morpho, Near Protocol, Ondo, OpenMind, Origin Protocol, Polkadot, Ripple, Solana Labs, StarkWare Industries, Stride, Subzero Labs, Terra, TON, Uniswap Labs, Unstoppable Domains, Wintermute, Worldcoin, Zcash (and others). Include funding rounds, partnerships, product launches, technical upgrades, governance proposals, acquisitions, or other significant business activities by these companies.
...
export async function scrapeMessariNews() {
  ...
  try {
    ...
    // Fallback: Try regular Messari data API first
    try {
      console.log('🔄 Falling back to regular Messari data API...');
      const { fetchMessariAPI } = await import('./messari-api.js');
      const articles = await fetchMessariAPI();
      
      if (articles && articles.length > 0) {
        // Use OpenAI to format the articles into a brief
        const { summarizeWithOpenAI } = await import('./openai.js');
        const formattedContent = await summarizeWithOpenAI(articles);
        
        return {
          content: formattedContent,
          sources: articles.map(a => a.url || a.source),
          generatedAt: new Date().toISOString()
        };
      }
    } catch (apiError) {
      console.log(`❌ Regular Messari API also failed: ${apiError.message}`);
    }
    
    // Final fallback: Generate basic message
