...
export async function formatNewsWithOpenAI(rawNewsContent) {
  ...
  try {
    ...
    const portfolioMentions = await detectPortfolioMentionsInText(rawNewsContent, PANTERA_PORTFOLIO_COMPANIES);
    console.log(`💎 Detected ${portfolioMentions.length} main topic portfolio companies`);
    
    // Enhanced logging for debugging
    if (portfolioMentions.length > 0) {
      console.log('📊 Main Topic Portfolio Companies:');
      portfolioMentions.forEach((mention, index) => {
...
Robinhood has completed a $200M acquisition of Bitstamp, with the crypto exchange securing a $40M Series A2 funding round as part of the deal. This acquisition expands Robinhood's global presence and provides additional liquidity infrastructure for retail crypto trading across international markets.

:two: *Expand Protocol Launches TREE Token with 75% APR Staking*

Expand has launched its TREE token on several major exchanges, offering staking vaults with up to 75% APR to attract liquidity providers. The protocol aims to build on-chain fixed income markets and expand its tAssets and derivatives offerings in the DeFi ecosystem.`
      : '';
...
- Do NOT include interactive language like "Let me know if you want more details" or "Feel free to ask" - this is an automated newsletter`
    ...
    const portfolioMentions = await detectPortfolioMentionsInText(rawNewsContent, PANTERA_PORTFOLIO_COMPANIES);
    console.log(`🔄 Using fallback formatter with ${portfolioMentions.length} main topic portfolio companies detected`);
    
    // Extract company names for fallback formatter compatibility
    const portfolioCompanyNames = portfolioMentions.map(mention => mention.company);
...
export async function summarizeWithOpenAI(articles, portfolioMentions = []) {
  ...
  const truncatedArticles = articles.map(a => ({
    title: a.title,
    summary: (a.summary || a.content || '').slice(0, 200) // Handle both summary and content fields
  }));
