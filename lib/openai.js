...
function formatMessariContentForSlack(rawContent, portfolioMentions, today) {
  ...
  // Remove markdown tables and convert to summary text
  const tableRegex = /\|\s*Asset.*?\|\s*\n\|[-:\s|]+\|\s*\n((?:\|.*?\|\s*\n)+)/gm;
  content = content.replace(tableRegex, (match) => {
    return 'Major cryptocurrencies showing strong trading volumes, led by Tether ($76B), Bitcoin ($27B), and Ethereum ($22B) in 24-hour volume.';
  });
  ...
  if (portfolioMentions.length > 0) {
    ...
    if (portfolioInMajorStories.length > 0) {
      const portfolioSection = `\n:gem: *Pantera Portfolio Highlights*\n\n\t• **${portfolioInMajorStories.slice(0, 3).join('**, **')}** featured in major developments\n\t• Strong institutional backing drives market leadership\n\t• Portfolio companies continue strategic expansion\n`;
...
- Use **bold headlines** for each story: "**Company Name: Brief Headline**" 
- Add 1-2 sentences of insight with metrics and context explaining why it matters
- Format like a polished financial news editor (Messari CoPilot style)
- Companies detected: ${portfolioMentions.join(', ')}
...
- CRITICAL: For proper Slack bullet formatting, each bullet point must start with a tab character followed by "• " (bullet + space). Each section with bullets must have a blank line after the header before the first bullet.
...
- Notable Events & Listings should be 4–6 bullet points, each starting with tab + "• " (tab + bullet + space)
- Infrastructure & Ecosystem should also have an emoji header with all items as tab + "• " bullet format
- EXAMPLE of correct Slack bullet format:
...
Note the blank line after the header before bullets start, and each bullet uses tab + bullet character.
