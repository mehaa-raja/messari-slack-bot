...
export async function formatNewsWithOpenAI(rawNewsContent, portfolioMentions = null) {
  ...
  try {
    ...
    const portfolioInstruction = finalPortfolioMentions.length > 0 
      ? `\n- PORTFOLIO HIGHLIGHTS SECTION: Include a dedicated section ":gem: *Pantera Portfolio Highlights*" with EXACTLY AND ONLY the following pre-verified companies. DO NOT add any other portfolio companies not listed below:
...
ABSOLUTE RULES FOR PORTFOLIO HIGHLIGHTS - NO EXCEPTIONS:
- You have exactly ${finalPortfolioMentions.length} pre-approved portfolio companies listed above
- Create exactly ${finalPortfolioMentions.length} portfolio highlights - no more, no less
- Use ONLY these ${finalPortfolioMentions.length} companies - ignore all other companies in the content
...
- FORBIDDEN: Adding companies like Babylon, Cardano, or any other company not in the numbered list above
- FORBIDDEN: Scanning the content for additional Pantera portfolio companies
- FORBIDDEN: Using your knowledge of Pantera's portfolio to add companies
...
:two: *Circle Announces Native USDC Minting on Hyperliquid*

*Circle (USDC):* Announced native USDC minting on Hyperliquid's HyperEVM blockchain and expanded stablecoin access via banking partnerships, opening stablecoin access to 280 banks and 90 million accounts.

NEVER INCLUDE:
- "Company X posts 7.16% gain" 
- "Token Y up 2.56%"
- "Shows no major developments"
- Pure price or trading volume updates without business context`
      : '\n- DO NOT include a Pantera Portfolio Highlights section - no pre-verified portfolio companies were found for today';
...
- CRITICAL FOR PORTFOLIO SECTION: Use ONLY the pre-verified portfolio companies provided in the instructions above`
        ...
        {
          role: "user", 
          content: `Please format this crypto news content for a professional daily newsletter. ${finalPortfolioMentions.length > 0 ? `Include ONLY these ${finalPortfolioMentions.length} portfolio companies in the portfolio section - do not add any others:` : ''}\n\n${rawNewsContent}`
        }
      ],
      max_tokens: 4000,
      temperature: 0.7
    ...
    // CRITICAL: Remove any portfolio companies that OpenAI added but weren't in our pre-detected list
    if (finalPortfolioMentions.length > 0) {
      const allowedCompanies = finalPortfolioMentions.map(m => m.company).join('|');
      console.log(`🔒 Allowed portfolio companies: ${allowedCompanies}`);
      
      // Clean up OpenAI's portfolio section formatting and remove duplicates
      const portfolioSectionRegex = /:gem: \*Pantera Portfolio Highlights\*[\s\S]*?(?=\n:[a-z_]+:|$)/g;
      const portfolioMatches = formattedContent.match(portfolioSectionRegex);
      
      if (portfolioMatches && portfolioMatches.length > 0) {
        // Take the LAST portfolio section (usually the cleanest one from OpenAI)
        const bestPortfolioSection = portfolioMatches[portfolioMatches.length - 1];
        
        // Remove ALL portfolio sections first
        formattedContent = formattedContent.replace(portfolioSectionRegex, '');
        
        // Clean up the formatting in the best section
        let cleanedSection = bestPortfolioSection
          .replace(/^#+\s*/gm, '') // Remove ### headers
          .replace(/\*\*([^*]+)\*\*/g, '*$1*') // Convert **bold** to *bold*
          .replace(/^[-*]+\s*/gm, '') // Remove leading dashes/asterisks from lines
          .replace(/\n{3,}/g, '\n\n') // Clean up excessive line breaks
          .trim();
        
        // Insert the cleaned section before Noteworthy Developments
        const beforeLastSection = formattedContent.lastIndexOf('\n:earth_americas:');
        if (beforeLastSection !== -1) {
          formattedContent = formattedContent.slice(0, beforeLastSection) + '\n\n' + cleanedSection + '\n' + formattedContent.slice(beforeLastSection);
        } else {
          // If no Noteworthy Developments section, append at the end
          formattedContent = formattedContent.trim() + '\n\n' + cleanedSection;
        }
        
        console.log(`✅ Cleaned and kept OpenAI's portfolio section (removed ${portfolioMatches.length - 1} duplicates)`);
      }
    }
